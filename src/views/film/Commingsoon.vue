...
Vue.filter('dateFilter', (date) => {
  // 日期处理函数 - moment.js
  return moment(date * 1000).locale('zh-cn').format('dddd M月 D日')
})
...
export default {
  ...
  mounted () {
    http({
      url: `/gateway?cityId=${this.cityId}&pageNum=1&pageSize=10&type=2&k=6556684`,
      headers: {
  ...
  methods: {
    onLoad () {
      ...
      // 1.ajax请求页面数据
      http({
        url: `/gateway?cityId=${this.cityId}&pageNum=${this.currentPage}&pageSize=10&type=2&k=6556684`,
        headers: {
          'X-Host': 'mall.film-ticket.film.list'
        }
      }).then(res => {
